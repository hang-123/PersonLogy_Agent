...
class _UnsupportedRetrievalReader:
    async def search(
        self,
        *,
        project_id: UUID,
        ...
        raise DomainValidationError(
            "retrieval is unavailable for the configured storage backend; "
            "use SQLite indexing or configure a Gel retrieval adapter"
        )
...
if isinstance(store, SQLiteStore):
    ...
    schema_service = SchemaChangeService(schema_registry, audit_sink=audit_sink)
    ...
    retrieval_reader = _UnsupportedRetrievalReader()
retrieval_service = RetrievalService(
    retrieval_reader, audit_sink=audit_sink, lineage_store=lineage_store, reranker=reranker
)
